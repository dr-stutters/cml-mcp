...
`ise_recent_authentications()` is the deployment-wide recent-auth report.

**Bulk endpoints + async tasks.** For mass MAB registration, `ise_bulk_create_endpoints(
macs, group_id=…)` / `ise_bulk_update_endpoints(macs, group_id=…)` (static-assign a group)
/ `ise_bulk_delete_endpoints(macs)` operate on a list of MACs in one call. Each returns
an **async task id** - poll it with `ise_get_task_status(task_id)` (executionStatus ->
COMPLETED_WITH_SUCCESS + success/fail counts). Same task-status tool tracks the
certificate-renew action.
