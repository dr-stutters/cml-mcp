...
**Stand up a domain.** `win_install_feature('AD-Domain-Services')` →
`win_promote_to_dc(domain_name='lab.local', safe_mode_password=...)` (reboots) →
after it's back, `win_ad_domain_info`, then create OUs/groups/users
(`win_create_ad_ou`, `win_create_ad_group`, `win_create_ad_user`).
