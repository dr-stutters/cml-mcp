...
> **INFRA DEPLOYED (2026-07-16):** **FMCv** (`198.18.128.80`, 32 GB) + **FTDv** (`198.18.128.81`, managed mode, day-0 `FmcIp=198.18.128.80`/`FmcRegKey=cisco123`) added to the SDA-Fabric lab, mgmt on the /18 via MGMT-SW. Host had ~89 GB free — no lab shutdown needed. **Day-0 gotcha:** FMCv `IPv4Mode` must be **`"manual"`** (not `"static"` — that silently leaves the mgmt IP blank; the console shows `Management IP: (empty)` and 8305 never opens). Corrected + rebooting. `.env` FMC creds updated (`admin`/`Cisc01@3`); the running `fmc` MCP won't hot-reload, so drive FMC via curl until restart. **NEXT:** FTD→FMC registration (gate on TCP 8305 per [[firepower-managed-mode-lab]]), a base ACP, then C5 (pxGrid SGT ISE→FMC), D5 (FMC→Splunk), and the FTD-in-fabric insertion at the inter-VN fusion hairpin (see B4) for the live SGACL drop.
