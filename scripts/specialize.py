...
Usage (from a fresh copy of the template directory):
    ...
    python3 scripts/specialize.py <service>     # e.g. cml, fmc, ise, catalyst_center
...
def main() -> None:
    if len(sys.argv) != 2:
        fail("usage: python3 scripts/specialize.py <service>  (e.g. cml, fmc, catalyst_center)")
    service = sys.argv[1].lower()
    if not re.fullmatch(r"[a-z][a-z0-9]*(_[a-z0-9]+)*", service):
        fail("service must be snake_case: lowercase letters/digits/underscores, e.g. catalyst_center")
