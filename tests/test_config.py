...
def test_reads_prefixed_env_vars(monkeypatch):
    monkeypatch.setenv("CML_MCP_BASE_URL", "https://box.example.com/api")
    monkeypatch.setenv("CML_MCP_ENABLE_WRITES", "true")
    monkeypatch.setenv("CML_MCP_VERIFY_TLS", "false")
...
def test_base_url_required(monkeypatch):
    monkeypatch.delenv("CML_MCP_BASE_URL", raising=False)
